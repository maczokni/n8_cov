...
forecast_chart <- function (forecasts, variable, value) {
  ...
  y_axis_label <- case_when(df_name %in% c("by_type_rt_forecast", 
                                           ...
                                           "domestic_by_grade_rt_forecast", 
                                           "domestic_by_grade_volume_forecasts") ~ "Median response time (in minutes)", 
                         df_name %in% c("call_forecasts") ~ "Weekly count of calls for service", 
                         df_name %in% c("by_type_attended_forecast", "by_grade_attended_forecast") ~ "Weekly % of calls attended")
  ...
  actual_col_name <- case_when(df_name %in% c("by_type_rt_forecast", 
                                           "by_grade_rt_forecast",
                                           "domestic_by_grade_rt_forecast", 
                                           "domestic_by_grade_volume_forecasts") ~ "actual_rt", 
                         df_name %in% c("call_forecasts") ~ "actual_calls", 
                         df_name %in% c("by_type_attended_forecast",
                                        "by_grade_attended_forecast") ~ "actual_attended")
  
  forecasts %>% 
    filter(eval(as.symbol(variable)) == value) %>% 
...
However when we break down by grade of call
...
Police response by grade. For grade 1, response time was within what was forecasted, however for grade 2 calls response times decreased during the pandemic. For grades 3 and 4 the forecase is too noisy (too few observations). 
