...
forecast_chart <- function (forecasts, variable, value) {
  ...
  y_axis_label <- case_when(df_name %in% c("by_type_rt_forecast", 
                                           "by_grade_rt_forecast") ~ "Median response time (in minutes)", 
  ...
  actual_col_name <- case_when(df_name %in% c("by_type_rt_forecast", 
                                           "by_grade_rt_forecast") ~ "actual_rt", 
                         df_name %in% c("call_forecasts") ~ "actual_calls", 
                         df_name %in% c("by_type_attended_forecast", "by_grade_attended_forecast") ~ "actual_attended")
...
Police response by grade

```{r}

# Attended by grade fig and response time by grade fig
