...
plot_imd_prepost <- function(inc_type) {
  ...
  mutate(pre_post = ifelse(incident_date_time < dmy("31/01/2020"), "Pre-COVID", "Post-COVID")) %>%   group_by(lsoa, pre_post) %>% count() %>% 
  ...
  mutate(n = replace_na(n, 0), 
         n_per_yr = ifelse(pre_post == "Pre-COVID", n/5,n)) 

labs_df <- pre_calls %>% 
  group_by(index_of_multiple_deprivation_imd_decile) %>% summarise(n = n_distinct(lsoa))

 p <-  ggplot() + 
  geom_boxplot(data = pre_calls, aes(x = as.factor(index_of_multiple_deprivation_imd_decile), y = n_per_yr, color = pre_post)) + 
  geom_label(data = labs_df, aes(x = index_of_multiple_deprivation_imd_decile, y = max(pre_calls$n_per_yr, na.rm = TRUE) + 10, label = paste0("n = ",n))) +
  labs(title = "Total calls from LSOAs in each IMD decile pre and post COVID") + 
  xlab("IMD decile (1 = most and 10 = least deprived)") + 
  ylab(paste0("Number of ",inc_type," calls (per year)")) + 
       scale_y_log10() + 
  theme_bw() + 
...
ggplot(pre_calls, aes(x = as.factor(index_of_multiple_deprivation_imd_decile), y = n_per_yr, color = pre_post)) + 
  geom_boxplot() + 
  geom_label(data = labs_df, aes(x = index_of_multiple_deprivation_imd_decile, y = max(pre_calls$n_per_yr + 100), label = paste0("n = ",n))) +
  labs(title = "Total calls from LSOAs in each IMD decile pre and post COVID") + 
  xlab("IMD decile (1 = most and 10 = least deprived)") + 
  ylab("Number of calls (per year)") + 
  theme_bw() + 
  theme(legend.position = c(0.87, 0.75))
