...
weekly_count_by_lsoa <- cheshire_calls %>% 
   mutate(inc_wk = yearweek(incident_date_time)) %>%   
  ...
  filter(!is.na(index_of_multiple_deprivation_imd_decile)) %>%  # check why NAs
  mutate(n = replace_na(n, 0)) %>% 
  group_by(index_of_multiple_deprivation_imd_decile, inc_wk) %>% 
  summarise(avg_weekly_call = mean(n, na.rm = T), 
            n_per_lsoa = n/n())
# 
# ggplot(weekly_count_by_lsoa, aes(x = inc_wk, y = avg_weekly_call, colour = as.factor(index_of_multiple_deprivation_imd_decile))) +
#   geom_point() + 
#   geom_line()
