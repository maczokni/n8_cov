...
forecast_chart_type <- function (forecasts, types) {
...
forecast_chart_grade <- function (forecasts, grade) {
  
  forecasts %>% 
    filter(initial_grade_of_response %in% grade) %>% 
    # Occasionally the ARIMA model may produce forecast confidence intervals
    # that are less than zero, which will not show up on the plot because we
    # have set the y axis to start at zero (to make changes in vertical position
    # on the axis representative of changes in magnitude). To deal with this we
    ...
    scale_fill_manual(values = c(`TRUE` = "black", `FALSE` = "grey80")) +
    labs(
      title = "Median response time all calls for service during 2020 compared to pre-pandemic forecast",
      subtitle = str_wrap(
        str_glue("Response time by week: ", str_c(pull(dates, label), collapse = "; ")), 
        80
      ),
...
```{r Admin}
forecast_chart_type(by_type_rt_forecast, "Advice/Directions/Admin")
```
...
```{r Alarms}
forecast_chart_type(by_type_rt_forecast, "alarms")
```

## Widlife Concern

```{r Wildlife}
forecast_chart_type(by_type_rt_forecast, "animals")
```
...
```{r ASB}
forecast_chart_type(by_type_rt_forecast, "ASB")
```
...
forecast_chart_type(by_type_rt_forecast, "Burglary - Business/Commercial")
```
...
forecast_chart_type(by_type_rt_forecast, "Burglary - Residential")
```
...
forecast_chart_type(by_type_rt_forecast, "Code Zero")
```
...
forecast_chart_type(by_type_rt_forecast, "Collapse/Illness/Injury/Trapped")
```
...
forecast_chart_type(by_type_rt_forecast, "Complaints Against Police")
```
...
forecast_chart_type(by_type_rt_forecast, "Concern For Safety")
```
...
```{r Criminal Damage}
forecast_chart_type(by_type_rt_forecast, "Criminal Damage")
```
...
```{r Domestic Incident}
forecast_chart_type(by_type_rt_forecast, "Domestic Incident")
```
...
forecast_chart_type(by_type_rt_forecast,"Drugs")
```
...
```{r Firearms}
forecast_chart_type(by_type_rt_forecast, "firearms")
```
...
```{r Fraud}
forecast_chart_type(by_type_rt_forecast,"Fraud")
```
...
```{r Hate Incident}
forecast_chart_type(by_type_rt_forecast,"Hate Incident")
```
...
forecast_chart_type(by_type_rt_forecast,"Highway Disruption")
```
...
```{r Missing Person}
forecast_chart_type(by_type_rt_forecast, "Missing Person")
```
...
```{r Other -Admin Calls}
forecast_chart_type(by_type_rt_forecast,"other - admin")
```
...
```{r Other - minor}
forecast_chart_type(by_type_rt_forecast, "other - minor")
```
...
```{r Police Generated}
forecast_chart_type(by_type_rt_forecast, "Police Generated")
```
...
```{r Breach of Peace}
forecast_chart_type(by_type_rt_forecast,"Prevent Potential Breach of the Peace")
```
...
forecast_chart_type(by_type_rt_forecast,"Property Lost/Found")
```
...
forecast_chart_type(by_type_rt_forecast,"Road Related Traffic Offence")
```
...
```{r Robbery}
forecast_chart_type(by_type_rt_forecast, "Robbery")
```
...
```{r Sexual Offences}
forecast_chart_type(by_type_rt_forecast, "Sexual Offences")
```
...
```{r Shoplifting}
forecast_chart_type(by_type_rt_forecast, "Shoplifting")
```
...
```{r Sudden Death}
forecast_chart_type(by_type_rt_forecast, "Sudden Death")
```
...
forecast_chart_type(by_type_rt_forecast, "Suspicious Activity")
```
...
forecast_chart_type(by_type_rt_forecast, "Suspicious Package")
```
...
forecast_chart_type(by_type_rt_forecast, "Theft From Motor Vehicle")
```
...
forecast_chart_type(by_type_rt_forecast, "Theft Of Motor Vehicle")
```
...
```{r Theft Other}
forecast_chart_type(by_type_rt_forecast, "Theft Other")
```
...
forecast_chart_type(by_type_rt_forecast, "traffic collision")
```
...
forecast_chart_type(by_type_rt_forecast, "Violence/Harassment")
```
...
```{r grade1}
forecast_chart_grade(by_grade_rt_forecast, "Grade 1")
```

## Grade 2

```{r grade2}
forecast_chart_grade(by_grade_rt_forecast, "Grade 2")
```

## Grade 3

```{r grade3}
forecast_chart_grade(by_grade_rt_forecast, "Grade 3")
```

## Grade 4

```{r grade4}
forecast_chart_grade(by_grade_rt_forecast, "Grade 4")
```

## Grade 5

```{r grade5}
forecast_chart_grade(by_grade_rt_forecast, "Grade 5")
```

## No Grade Specified

```{r nogradespec}
forecast_chart_grade(by_grade_rt_forecast, "No Grade Specified")
```

## No Grade Recorded

```{r gradenotrec}
forecast_chart_grade(by_grade_rt_forecast, "Not Recorded")
```
